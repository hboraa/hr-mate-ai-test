...
function App() {
  // Tabs: 'personal' (Home), 'policy' (Docs), 'onboarding', 'admin', 'detail' (AI Detail), 'notice', 'notice-detail', 'leave'
  const [activeTab, setActiveTab] = useState<'personal' | 'policy' | 'admin' | 'onboarding' | 'detail' | 'notice' | 'notice-detail' | 'leave'>('personal');
  ...
  const [detailContent, setDetailContent] = useState<string>(''); // For AI detailed response
  ...
  const handleOpenDetail = (content: string) => {
  ...
  const handleSelectPolicy = (id: string) => {
    if (id === '') {
      ...
      setSelectedPolicyId(id);
      setActiveTab('policy'); // Switch to policy tab when a policy is selected
    }
  }
      ...
      <main className="flex-1 flex relative overflow-hidden">
        ...
        <div className="flex-1 h-full w-full bg-slate-50 relative">
          <Dashboard
            ...
            onViewLeaveDetail={handleViewLeaveDetail}
...
const NavButton = ({ active, onClick, icon, label }: { active: boolean, onClick: () => void, icon: React.ReactNode, label: string }) => (
  <button
    onClick={onClick}
    className={`flex flex-col items-center gap-1 p-2 rounded-xl transition-all w-16 group ${active ? 'text-blue-400 bg-slate-800' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800/50'
      }`}
  >
    {icon}
