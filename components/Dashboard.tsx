...
interface DashboardProps {
  user: User;
  activeTab: 'personal' | 'policy' | 'admin' | 'onboarding';
  selectedPolicyId?: string;
  onSelectPolicy: (id: string) => void;
}

const Dashboard: React.FC<DashboardProps> = ({ user, activeTab, selectedPolicyId, onSelectPolicy }) => {
  
  const renderContent = () => {
    switch (activeTab) {
      case 'personal':
        return <PersonalTab user={user} onSelectPolicy={onSelectPolicy} />;
      case 'policy':
...
const PersonalTab = ({ user, onSelectPolicy }: { user: User, onSelectPolicy: (id: string) => void }) => (
  <div className="p-6 space-y-6">
    ...
    <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 relative overflow-hidden">
      ...
      <div className="grid grid-cols-2 gap-4 relative z-10">
        <div 
          onClick={() => onSelectPolicy('leave-01')} 
        ...
        <div className="bg-purple-50/80 p-4 rounded-xl border border-purple-100">
           ...
           <div className="flex items-baseline gap-1">
             <span className="text-2xl font-extrabold text-purple-800">12.25</span>
             <span className="text-sm text-purple-600 font-medium">(D-10)</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100">
      <div className="flex items-center justify-between mb-4">
        ...
        <button className="text-xs text-slate-400 hover:text-blue-600">전체보기</button>
      </div>
      
      <div className="space-y-3">
         {MOCK_NOTICES.map((notice) => (
           <div key={notice.id} className="flex items-start gap-3 p-2 rounded-lg hover:bg-slate-50 transition-colors cursor-pointer group">
    ...
    return (
      <div className="p-6 h-full flex flex-col animate-fade-in-up">
        ...
        <div className="mt-4 flex justify-end">
           <button className="flex items-center gap-2 text-slate-600 bg-white border border-slate-200 px-4 py-2 rounded-lg text-sm font-medium hover:bg-slate-50 hover:text-blue-600 transition-colors shadow-sm">
             <Download size={16} />
             PDF 다운로드
           </button>
        </div>
      ...
      {[
        ...
        <div key={idx} className="relative pl-8 group">
           <div className={`absolute -left-[9px] top-1 w-4 h-4 rounded-full border-2 transition-all ${
             step.status === 'done' ? 'bg-green-500 border-green-500 scale-110' : 
           ...
           <div className={`p-4 rounded-xl border transition-all ${
             step.status === 'current' ? 'bg-white border-blue-200 shadow-sm' : 'border-transparent hover:bg-white hover:border-slate-100'
           }`}>
